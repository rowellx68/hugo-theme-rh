namespace HugoThemeRh {
  ...
  function socialShareUtil(site: string, url: string, shareUrl: string) {
    const win = window.open(
      shareUrl,
      "_blank",
      "height=400, width=500, menubar=0"
    );
  }
  ...
  function socialShareUtilBuildUrl(site: string, title: string, url: string) {
    switch (site) {
      case "twitter":
        return `${ShareBaseLinks.twitter}?url=${encodeURIComponent(
          url
        )}&text=${encodeURIComponent(
          title
        )}&original_referer=${encodeURIComponent(url)}`;
      case "facebook":
        return `${ShareBaseLinks.facebook}?u=${encodeURIComponent(
          url
        )}&title=${encodeURIComponent(title)}`;
      case "linkedin":
        return `${ShareBaseLinks.linkedin}?url=${encodeURIComponent(
          url
        )}&title=${encodeURIComponent(title)}`;
      case "google":
  ...
  export class Rh {
    start() {
      ...
      this.attachEventScrollTop();
    }

    private attachEventNavToggle() {
      const navToggles: [HTMLElement] = Array.prototype.slice.call(
    ...
    private attachEventShareLinks() {
      const social: [HTMLElement] = Array.prototype.slice.call(
        document.getElementsByClassName("social-share-link")
      );
        ...
        if (
          ...
          el.addEventListener("click", ev => {
            ...
            socialShareUtil(
              social,
              url,
              socialShareUtilBuildUrl(site, title, url)
            );
          });
        }
      });
    ...
    private attachEventScrollTop() {
      const scrollTop: [HTMLElement] = Array.prototype.slice.call(
        document.getElementsByClassName("scroll-top")
      );
  ...
  export function executeIfElementWithIdExists(
    elementId: string,
    callback: Function
  ) {
